...
fn main() {
    ...
    roots_of_unity.iter().for_each(|&v| {
        let bytes = unsafe { std::mem::transmute::<Scalar, [u8; 32]>(v) };
        roots_of_unity_bytes.extend_from_slice(&bytes);
    });

    g1_points.iter().for_each(|&v| {
        let bytes = unsafe { std::mem::transmute::<G1Affine, [u8; 104]>(v) };
        g1_bytes.extend_from_slice(&bytes);
    });

    g2_points.iter().for_each(|&v| {
        let bytes = unsafe { std::mem::transmute::<G2Affine, [u8; 200]>(v) };
        g2_bytes.extend_from_slice(&bytes);
    });
