...
pub(crate) mod owned {
    ...
    impl MptOwned {
        pub(crate) fn decode_from_proof_rlp(bytes: &mut &[u8]) -> Result<Self, Error> {
            ...
            // SAFETY: `bytes` was allocated in the leaked bump arena, so it lives for the
            // lifetime of the owned trie.
        ...
        fn alloc_in_bump(&self, bytes: &[u8]) -> &'static [u8] {
            ...
            // SAFETY: `slice` lives as long as `self.inner.bump`, which is leaked for
            // the lifetime of this owned trie.
