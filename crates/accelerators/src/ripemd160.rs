//! RIPEMD-160 accelerator.

use crate::types::{zkvm_status, ZkvmBytes, ZKVM_EFAIL, ZKVM_EOK};

pub type zkvm_ripemd160_hash = ZkvmBytes<32>;
...
/// - `output`, if non-NULL, must be valid for writes of one [`zkvm_ripemd160_hash`].
#[unsafe(no_mangle)]
pub unsafe extern "C" fn zkvm_ripemd160(
    ...
    let value = zkvm_ripemd160_hash { data: ripemd160(data) };
