...
echo "running: RUSTFLAGS=\"$RUSTFLAGS\" cargo \"$TOOLCHAIN\" build -p static-verifier-tracegen --profile=profiling --no-default-features --features $FEATURES"

RUSTFLAGS="$RUSTFLAGS" cargo "$TOOLCHAIN" build \
  -p static-verifier-tracegen \
  --profile=profiling \
  --no-default-features \
  --features "$FEATURES"

BIN="$REPO_ROOT/target/profiling/static-verifier-tracegen"
...
if [ "$USE_NSYS" = "true" ]; then
  NSYS_OUTPUT="$REPO_ROOT/tracegen.nsys-rep"
  ...
  PERF_OUTPUT="$REPO_ROOT/tracegen-perf.data"
  SAMPLY_DIR="$REPO_ROOT/samply_profile"
  SAMPLY_OUTPUT="$SAMPLY_DIR/tracegen-profile.json.gz"
  mkdir -p "$SAMPLY_DIR"
  ...
  COZ_OUTPUT="$REPO_ROOT/tracegen.coz"
  ...
  echo "View with: coz plot $COZ_OUTPUT   (or upload to https://plasma-umass.org/coz/)"
else
