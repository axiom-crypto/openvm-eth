...
use crate::transcript::{digest_wire_from_root, TranscriptChip};
use crate::verify::batch_constraints::{eval_eq_mle_assigned, eval_eq_mle_ef_f_assigned};
use crate::verify::{horner_eval_ext_poly_assigned, horner_eval_ext_poly_f_assigned, interpolate_quadratic_at_012_assigned};
...
pub(crate) fn load_whir_proof_wire<E: BabyBearExtInst>(
    ...
    let folding_pow_witnesses = whir_proof
        ...
        .iter()
        .map(|&w| base_chip.load_reduced_witness(RootF::from_u64(w.as_canonical_u64())))
        .collect::<Vec<_>>();
    ...
    let whir_sumcheck_polys = whir_proof
        ...
        .map(|poly| {
            poly.iter()
                .map(|&v| ext_chip.load_reduced_witness(v))
                .collect::<Vec<_>>()
                .try_into()
...
fn eval_mobius_eq_mle_assigned<E: BabyBearExtInst>(
    ...
    for (u_i, x_i) in u.iter().zip(x.iter()) {
        ...
        let three_x_minus_two = ext_chip.mul_base_const(*x_i, three);
        let three_x_minus_two = ext_chip.sub(three_x_minus_two, two);
        let u_term = ext_chip.mul(*u_i, three_x_minus_two);
        let factor = ext_chip.add(one_minus_x, u_term);
...
fn eval_mle_evals_at_point_assigned<E: BabyBearExtInst>(
    ...
    for xj in x.iter().rev() {
        ...
        for i in 0..len {
            let lo = values[i];
            let hi = values[i + len];
            let diff = ext_chip.sub(hi, lo);
            let weighted = ext_chip.mul(diff, *xj);
            values[i] = ext_chip.add(lo, weighted);
        }
    }
...
fn binary_k_fold_assigned<E: BabyBearExtInst>(
    ...
    let mut x_pow = base_chip.reduce_max_bits(x);
    let x_inv = invert_base_assigned(base_chip, x);
    let mut x_inv_pow = base_chip.reduce_max_bits(x_inv);

    for (j, alpha) in alphas.iter().enumerate() {
        let m = n >> (j + 1);
        for i in 0..m {
            let t = base_chip.mul_const(x_pow, tw[i << j]);
            let t_inv_half = base_chip.mul_const(x_inv_pow, inv_tw_half[i << j]);

            let lo = values[i];
            let hi = values[i + m];
            let lo_minus_hi = ext_chip.sub(lo, hi);
            let mut alpha_minus_t = *alpha;
            alpha_minus_t.0[0] = base_chip.sub(alpha_minus_t.0[0], t);
            let fold = ext_chip.mul(alpha_minus_t, lo_minus_hi);
            values[i] = ext_chip.scalar_mul_add(fold, t_inv_half, lo);
        }
        x_pow = base_chip.square(x_pow);
        x_pow = base_chip.reduce_max_bits(x_pow);
        x_inv_pow = base_chip.square(x_inv_pow);
        x_inv_pow = base_chip.reduce_max_bits(x_inv_pow);
    }
    values[0]
}
...
fn tree_compress_assigned_digests<R: crate::repr::FieldRepr>(digests: Vec<R>) -> R {
    ...
    while level.len() > 1 {
        ...
        for pair in level.chunks_exact(2) {
            next.push(compress_bn254_digests(pair[0], pair[1]));
        }
        level = next;
    }
    level.pop().unwrap()
...
fn constrain_merkle_path<E: BabyBearExtInst>(
    ...
    let leaf_hashes = merkle_path
        .leaf_values
        .iter()
        .map(|leaf| hash_babybear_slice_to_digest(gate, leaf))
        .collect::<Vec<_>>();
...
pub(crate) fn constrain_whir_verification<E: BabyBearExtInst>(
    ext_chip: &E,
    ...
    E::Base: Clone,
{
    ...
    transcript.check_witness(params.whir.mu_pow_bits, &mu_pow_witness);
    let mu_challenge = transcript.sample_ext();
    ...
    for _ in 0..total_width {
        mu_pows.push(mu_pow);
        mu_pow = ext_chip.mul(mu_pow, mu_challenge);
        mu_pow = ext_chip.reduce_max_bits(mu_pow);
    }

    let mut final_claim = ext_chip.zero();
    ...
    for commit_openings in stacking_openings {
        for opening in commit_openings {
            let weighted = if mu_idx == 0 {
                ...
                ext_chip.mul(opening.into(), mu_pows[mu_idx])
            };
            final_claim = ext_chip.add(final_claim, weighted);
            mu_idx += 1;
        }
    }
    ...
    for (round_idx, round_params) in params.whir.rounds.iter().enumerate() {
        ...
        for _ in 0..k_whir {
            if let Some(evals) = whir_sumcheck_polys.get(sumcheck_cursor) {
                ...
                transcript.check_witness(params.whir.folding_pow_bits, &pow_witness);
        ...
        let y0 = if is_final_round {
            ...
            transcript.observe_commit(&codeword_commitment_digests[round_idx]);
        ...
        for query_idx in 0..num_queries {
            ...
            let yi = if is_initial_round {
                    ...
                    for col_idx in 0..commit_openings.len() {
                        ...
                        for (row_idx, row) in merkle_path.leaf_values.iter().enumerate() {
                            ...
                            codeword_vals[row_idx] = if let Some(prev) = codeword_vals[row_idx] {
                                ...
                                Some(ext_chip.from_base_var(opened_base))
                            } else {
                                Some(ext_chip.scalar_mul(mu_pow, opened_base))
                            };
                        }
                        mu_power_idx += 1;
                    }
                ...
                binary_k_fold_assigned(ext_chip, codeword_vals, &alphas_round, zi_root)
            } else {
        ...
        if let Some(y0) = y0 {
            let y0_term = ext_chip.mul(y0.into(), gamma);
            final_claim = ext_chip.add(final_claim, y0_term);
        }
        let mut gamma_pow = ext_chip.mul(gamma, gamma);
        for yi in &ys_round {
            let term = ext_chip.mul(*yi, gamma_pow);
            final_claim = ext_chip.add(final_claim, term);
            gamma_pow = ext_chip.mul(gamma_pow, gamma);
        }
        gammas.push(gamma);
    ...
    let prefix = eval_mobius_eq_mle_assigned(ext_chip, &u_cube[..t], &folding_alphas[..t]);
    let suffix = eval_mle_evals_at_point_assigned(ext_chip, &final_poly, &u_cube[t..]);
    let mut final_acc = ext_chip.mul(prefix, suffix);
    ...
    for round_idx in 0..rounds {
        ...
        if round_idx + 1 != rounds {
            ...
            let poly_eval = horner_eval_ext_poly_assigned(ext_chip, &final_poly, &z0_max);
            let term = ext_chip.mul(*gamma, eq);
            let term = ext_chip.mul(term, poly_eval);
            final_acc = ext_chip.add(final_acc, term);
        }
        ...
        for zi in zs_per_round[round_idx].iter() {
            ...
            for _ in 1..slc_len {
                ...
                let next = ext_chip.base().square(*prev);
                zi_pows.push(next);
            }
            let zi_pows_reduced: Vec<_> = zi_pows
                .iter()
                .map(|p| ext_chip.base().reduce_max_bits(*p))
                .collect();
            ...
            let term = ext_chip.mul(gamma_pow, eq);
            let term = ext_chip.mul(term, poly_eval);
            final_acc = ext_chip.add(final_acc, term);
            gamma_pow = ext_chip.mul(gamma_pow, *gamma);
        }
    ...
    ext_chip.assert_equal(final_acc, final_claim);
}
