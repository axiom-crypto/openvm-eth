...
use crate::{
    ...
    proof_wire::{BatchConstraintIntermediatesWire, BatchConstraintProofWire, GkrProofWire},
    transcript::TranscriptChip,
    verify::{column_openings_by_rot_assigned, horner_eval_ext_poly_assigned},
    wire::{ExtWire, ReducedExtWire, ReducedWire, Wire},
};

pub(crate) fn load_gkr_proof_wire<B: BabyBearExt4Inst>(
    ctx: &mut B::Ctx,
    ...
    let logup_pow_witness = base.load_reduced_witness(ctx, gkr_proof.logup_pow_witness);
    ...
    let sumcheck_polys = gkr_proof
        ...
        .collect::<Vec<_>>();
    GkrProofWire { logup_pow_witness, q0_claim, claims_per_layer, sumcheck_polys }
}

pub(crate) fn load_batch_constraint_proof_wire<B: BabyBearExt4Inst>(
    ctx: &mut B::Ctx,
    ext_chip: &BabyBearExt4Chip<B>,
    batch_proof: &openvm_stark_sdk::openvm_stark_backend::proof::BatchConstraintProof<RootConfig>,
) -> BatchConstraintProofWire<B> {
    ...
    let sumcheck_round_polys = batch_proof
        ...
        .iter()
        .map(|poly| {
            poly.iter().map(|&value| ext_chip.load_reduced_witness(ctx, value)).collect::<Vec<_>>()
        })
        .collect::<Vec<_>>();
...
fn eval_lagrange_on_integer_grid<B: BabyBearExt4Inst>(
    ...
    let x_grid = (0..=n)
        .map(|j| ext_chip.base().load_constant(ctx, RootF::from_u64(j as u64)))
        .collect::<Vec<_>>();
    ...
    for (i, eval_i) in evals.iter().enumerate() {
        ...
        for j in 0..=n {
            ...
            let diff = if i >= j { RootF::from_usize(i - j) } else { -RootF::from_usize(j - i) };
            denom *= diff;
        }
...
pub(crate) fn eval_eq_mle_binary_assigned<B: BabyBearExt4Inst>(
    ...
    for (x_i, bit) in x.iter().zip(y_bits.iter().copied()) {
        let factor = if bit { *x_i } else { ext_chip.sub(ctx, one, *x_i) };
        acc = ext_chip.mul(ctx, acc, factor);
    }
    acc
}
...
#[derive(Clone)]
struct ViewPairWire<B: BabyBearExt4Inst> {
    local: ExtWire<B>,
    next: ExtWire<B>,
}

impl<B: BabyBearExt4Inst> From<(ExtWire<B>, ExtWire<B>)> for ViewPairWire<B> {
    fn from((local, next): (ExtWire<B>, ExtWire<B>)) -> Self {
        Self { local, next }
    }
}

struct ConstraintEvaluatorWire<'a, B: BabyBearExt4Inst> {
    preprocessed: Option<&'a [ViewPairWire<B>]>,
    partitioned_main: &'a [Vec<ViewPairWire<B>>],
    is_first_row: ExtWire<B>,
    is_last_row: ExtWire<B>,
    public_values: &'a [ReducedWire<B>],
}

impl<B: BabyBearExt4Inst> ConstraintEvaluatorWire<'_, B> {
    fn eval_var(
        &self,
        ctx: &mut B::Ctx,
        ext_chip: &BabyBearExt4Chip<B>,
...
fn eval_symbolic_nodes_assigned<B: BabyBearExt4Inst>(
    ctx: &mut B::Ctx,
    ext_chip: &BabyBearExt4Chip<B>,
    evaluator: &ConstraintEvaluatorWire<'_, B>,
    nodes: &[SymbolicExpressionNode<RootF>],
) -> Vec<ExtWire<B>> {
    let mut exprs: Vec<ExtWire<B>> = Vec::with_capacity(nodes.len());
    for node in nodes {
        let expr = match node {
...
fn local_next_opening_views<B: BabyBearExt4Inst>(
    ctx: &mut B::Ctx,
    ext_chip: &BabyBearExt4Chip<B>,
    openings: &[ReducedExtWire<B>],
    need_rot: bool,
) -> Vec<ViewPairWire<B>> {
    let openings = openings.iter().map(ExtWire::<B>::from).collect::<Vec<_>>();
...
fn observe_layer_claims_assigned<B: BabyBearExt4Inst>(
    ctx: &mut B::Ctx,
    transcript: &mut TranscriptChip<B>,
    claims: &[ReducedExtWire<B>],
) {
    for claim in claims {
...
pub(crate) fn constrain_batch_constraints_verification<B: BabyBearExt4Inst>(
    ctx: &mut B::Ctx,
    ext_chip: &BabyBearExt4Chip<B>,
    transcript: &mut TranscriptChip<B>,
    mvk0: &MultiStarkVerifyingKey0<RootConfig>,
    gkr_wire: &GkrProofWire<B>,
    batch_wire: &BatchConstraintProofWire<B>,
    n_per_trace: &[isize],
    trace_id_to_air_id: &[usize],
    public_values: Vec<Vec<ReducedWire<B>>>,
) -> BatchConstraintIntermediatesWire<B>
where
    B: Clone,
{
    ...
    let total_interactions_host = zip(trace_id_to_air_id, n_per_trace)
        .map(|(&air_idx, &n)| {
            ...
            let num_interactions = mvk0.per_air[air_idx].symbolic_constraints.interactions.len();
    ...
    let trace_constraint_nodes = trace_id_to_air_id
        .iter()
        .map(|&air_id| mvk0.per_air[air_id].symbolic_constraints.constraints.nodes.clone())
        .collect::<Vec<_>>();
    let trace_constraint_indices = trace_id_to_air_id
        .iter()
        .map(|&air_id| mvk0.per_air[air_id].symbolic_constraints.constraints.constraint_idx.clone())
        .collect::<Vec<_>>();
    let trace_interactions = trace_id_to_air_id
        .iter()
        .map(|&air_id| mvk0.per_air[air_id].symbolic_constraints.interactions.clone())
        .collect::<Vec<_>>();
    ...
    let (mut gkr_p_xi_claim, mut gkr_q_xi_claim, xi) = {
        ...
        let layer0_p0: ExtWire<B> = (&layer0[0]).into();
        let layer0_q0: ExtWire<B> = (&layer0[1]).into();
        let layer0_p1: ExtWire<B> = (&layer0[2]).into();
        let layer0_q1: ExtWire<B> = (&layer0[3]).into();
        ...
        for round in 1..total_gkr_rounds {
            ...
            for (subround, xi_prev) in gkr_r.iter().enumerate().take(round) {
                let [ev1, ev2, ev3]: [ReducedExtWire<B>; 3] = round_polys[subround];
            ...
            let layer_p0: ExtWire<B> = (&layer_claims[0]).into();
            let layer_q0: ExtWire<B> = (&layer_claims[1]).into();
            let layer_p1: ExtWire<B> = (&layer_claims[2]).into();
            let layer_q1: ExtWire<B> = (&layer_claims[3]).into();
    ...
    for (num_term, den_term) in numerator_term_per_air.iter().zip(denominator_term_per_air.iter()) {
    ...
    for (num_term, den_term) in numerator_term_per_air.iter().zip(denominator_term_per_air.iter()) {
        let num_term: ExtWire<B> = num_term.into();
        let den_term: ExtWire<B> = den_term.into();
    ...
    let univariate_round_coeffs_raw: Vec<ExtWire<B>> =
        univariate_round_coeffs.iter().map(ExtWire::<B>::from).collect();
    ...
    for round_evals in sumcheck_round_polys {
        ...
        let s_1: ExtWire<B> = (&round_evals[0]).into();
        ...
        interpolation_evals.extend(round_evals.iter().map(ExtWire::<B>::from));
    ...
    let reduced_zero = ext_chip
        .load_reduced_constant(ctx, openvm_stark_sdk::config::baby_bear_bn254_poseidon2::EF::ZERO);
    ...
    for (trace_idx, &n) in n_per_trace.iter().enumerate() {
        ...
        let factors: Vec<(ExtWire<B>, ExtWire<B>)> = xi_slice
            .iter()
            .map(|x_i| {
        ...
        let mut prev_level: BTreeMap<usize, ExtWire<B>> = BTreeMap::new();
    ...
    for (i, r_i) in r.iter().enumerate().skip(1) {
        let eq_mle =
            eval_eq_mle_assigned(ctx, ext_chip, &[xi[l_skip + i - 1]], core::slice::from_ref(r_i));
    ...
    let mut cur_mu_pow = one;
    for (i, term) in interactions_evals.iter().chain(constraints_evals.iter()).enumerate() {
        let weighted_term = if i == 0 { *term } else { ext_chip.mul(ctx, *term, cur_mu_pow) };
        consistency_rhs = ext_chip.add(ctx, consistency_rhs, weighted_term);
        cur_mu_pow = ext_chip.mul(ctx, cur_mu_pow, mu);
    }
    ext_chip.assert_equal(ctx, consistency_lhs, consistency_rhs);

    BatchConstraintIntermediatesWire { column_openings: column_openings.clone(), r }
}
