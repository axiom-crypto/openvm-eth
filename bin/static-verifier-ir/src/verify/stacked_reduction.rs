...
use crate::{
    ...
    proof_wire::{StackedReductionIntermediatesWire, StackingProofWire},
    ...
    wire::{ExtWire, ReducedExtWire},
};

pub(crate) fn load_stacking_proof_wire<B: BabyBearExt4Inst>(
    ctx: &mut B::Ctx,
    ...
    let stacking_openings = stacking_proof
        ...
        .iter()
        .map(|row| {
            row.iter().map(|&value| ext_chip.load_reduced_witness(ctx, value)).collect::<Vec<_>>()
        })
        .collect::<Vec<_>>();
    StackingProofWire { univariate_round_coeffs, sumcheck_round_polys, stacking_openings }
}
...
pub(crate) fn constrain_stacked_reduction<B: BabyBearExt4Inst>(
    ctx: &mut B::Ctx,
    ext_chip: &BabyBearExt4Chip<B>,
    transcript: &mut TranscriptChip<B>,
    stacking_wire: &StackingProofWire<B>,
    layouts: &[StackedLayout],
    need_rot_per_commit: &[Vec<bool>],
    l_skip: usize,
    n_stack: usize,
    batch_column_openings: &[Vec<Vec<ReducedExtWire<B>>>],
    r: &[ExtWire<B>],
) -> StackedReductionIntermediatesWire<B>
where
    B: Clone,
{
    ...
    for (trace_idx, parts) in batch_column_openings.iter().enumerate() {
        ...
        let openings = parts[0].iter().map(ExtWire::<B>::from).collect::<Vec<_>>();
        t_claims.extend(column_openings_by_rot_assigned(ctx, ext_chip, &openings, need_rot));
    }
    ...
    for parts in batch_column_openings {
        for cols in parts.iter().skip(1) {
            ...
            let openings = cols.iter().map(ExtWire::<B>::from).collect::<Vec<_>>();
            t_claims.extend(column_openings_by_rot_assigned(ctx, ext_chip, &openings, need_rot));
            commit_idx += 1;
        }
    }
        ...
        t_claims.iter().zip(lambda_sqr_powers.iter()).enumerate()
    {
        ...
        let term =
            if i == 0 { batched_claim } else { ext_chip.mul(ctx, batched_claim, *lambda_pow) };
    ...
    for round_poly in sumcheck_round_polys {
        ...
        let s_j_1: ExtWire<B> = (&s_j_1).into();
        let s_j_2: ExtWire<B> = (&s_j_2).into();
    ...
    let mut derived_q_coeffs = stacking_matrix_expected_widths
        .iter()
        .map(|&width| {
            core::iter::repeat_with(|| ext_chip.zero(ctx)).take(width).collect::<Vec<_>>()
        })
        .collect::<Vec<_>>();

    let any_need_rot =
        lambda_indices_per_layout.iter().any(|indices| indices.iter().any(|&(_, rot)| rot));

    let mut n_cache: HashMap<isize, (ExtWire<B>, ExtWire<B>, ExtWire<B>)> = HashMap::new();
    let mut eq_mle_cache: HashMap<(isize, usize), ExtWire<B>> = HashMap::new();
    ...
    StackedReductionIntermediatesWire { stacking_openings: stacking_openings.clone(), u }
}
