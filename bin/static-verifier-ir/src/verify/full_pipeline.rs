...
use crate::{
    ...
    verify::{
        ...
        whir::{constrain_whir_verification, load_whir_proof_wire},
    },
    wire::ReducedWire,
};
...
pub fn load_proof_wire<B: BabyBearExt4Inst>(
    ctx: &mut B::Ctx,
    ...
    let common_main_commit_root = B::input(ctx, digest_scalar_to_fr(proof.common_main_commit[0]));
        ...
        .map(|vdata| {
            if let Some(vdata) = vdata {
                vdata
                    .cached_commitments
                    .iter()
                    .map(|commit| B::input(ctx, digest_scalar_to_fr(commit[0])))
                    .collect::<Vec<_>>()
            } else {
                Vec::new()
            }
    ...
    let gkr = load_gkr_proof_wire(ctx, ext_chip, &proof.gkr_proof);
    ...
    let whir = load_whir_proof_wire(ctx, ext_chip, &proof.whir_proof);
...
fn observe_preamble<B: BabyBearExt4Inst>(
    ctx: &mut B::Ctx,
    transcript: &mut TranscriptChip<B>,
    mvk: &MultiStarkVerifyingKey<RootConfig>,
    log_heights_per_air: &[usize],
    public_values: &[Vec<ReducedWire<B>>],
    cached_commitment_roots: &[Vec<B::V>],
    vk_pre_hash: DigestWire<B>,
    common_main_commit: DigestWire<B>,
) {
    ...
    for air_idx in 0..mvk.inner.per_air.len() {
        if !mvk.inner.per_air[air_idx].is_required {
            let presence_flag = transcript.baby_bear().load_reduced_constant(ctx, BabyBear::ONE);
            transcript.observe(ctx, &presence_flag);
        }

        if let Some(preprocessed) = mvk.inner.per_air[air_idx].preprocessed_data.as_ref() {
            let preprocessed_root = B::constant(ctx, digest_scalar_to_fr(preprocessed.commit[0]));
            transcript.observe_commit(ctx, &digest_wire_from_root::<B>(preprocessed_root));
        } else {
            let lh = u32::try_from(log_heights_per_air[air_idx]).expect("log_height must fit u32");
            let log_height =
                transcript.baby_bear().load_reduced_constant(ctx, BabyBear::from_u32(lh));
            transcript.observe(ctx, &log_height);
        }

        for root in &cached_commitment_roots[air_idx] {
            transcript.observe_commit(ctx, &digest_wire_from_root::<B>(*root));
        }
...
pub fn constrained_verify<B: BabyBearExt4Inst>(
    ctx: &mut B::Ctx,
    ext_chip: &BabyBearExt4Chip<B>,
    root_vk: &MultiStarkVerifyingKey<RootConfig>,
    proof_wire: &ProofWire<B>,
    ...
    let mvk_pre_hash_root = B::constant(ctx, digest_scalar_to_fr(root_vk.pre_hash[0]));
    ...
    observe_preamble(
        ...
        digest_wire_from_root::<B>(mvk_pre_hash_root),
        digest_wire_from_root::<B>(proof_wire.common_main_commit_root),
    );
    ...
    let initial_commitment_roots = {
        ...
        for &air_id in trace_id_to_air_id {
            if let Some(preprocessed) = &root_vk.inner.per_air[air_id].preprocessed_data {
                commits.push(B::constant(ctx, digest_scalar_to_fr(preprocessed.commit[0])));
            }
